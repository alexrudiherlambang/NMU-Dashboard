...
class ckunjungan_BPJS extends CI_Controller {
   ...
   function grafik_kunjungan_bpjs() {
      ...
      $nama = $this->session->userdata("nama");
      $data['jenis'] = $this->mkunjungan_BPJS->mshow_all_jenis($nama);
      $this->load->view('content/vunit/vkunjungan_bpjs/vgrafik_kunjungan_bpjs', $data);
   }
   ...
   function grafik_hasil_kunjungan() {
      ...
      if ($jenis == "SEMUA") {
         $grafik = $this->mkunjungan_BPJS->mshow_all_grafik_all_jenis($tglawal,$tglakhir,$lokasi,$jenis,$nama);
         ...
         $graf_pie = $this->mkunjungan_BPJS->mshow_all_pie_all_jenis($tglawal,$tglakhir,$lokasi,$jenis,$nama);
         ...
         if (empty($hasiltanggal)) {
            ...
            $this->mkunjungan_BPJS->insert_log($log);
            $this->load->view('content/vunit/vkunjungan_BPJS/vgrafik_hasil_kunjungan_BPJS_all',$data);
         }
      }else{
         ...
         if (empty($hasiltanggal)) {
            ...
            $this->load->view('content/vunit/vkunjungan_BPJS/vgrafik_hasil_kunjungan_BPJS',$data);
         }
      }
   }
   ...
   function export_xls() {
      ...
      if (isset($_POST['exportType'])) {
         ...
         if ($exportType == 'detail') {
            ...
            $this->load->helper('exportexcel');
            $namaFile = "Tabel Kunjungan BPJS / NON BPJS.xls.xls";
            $judul = "Tabel Kunjungan BPJS / NON BPJS.xls";
            $tablehead = 0;
            $tablebody = 1;
            $nourut = 1;
            //penulisan header
            ...
            $total_jmltarget = 0;
            $total_jmlprosen = 0;
            foreach ($this->mkunjungan_BPJS->mshow_all_call($tglawal,$tglakhir,$nama,$lokasi) as $data) {
