...
class mlaba_rugi extends ci_model {
   ...
   // untuk cetak potensi
   function mshow_all_potensi($nama, $kellabarugi, $tglawal, $tglakhir) {
      $this->db->select('lokasi,tanggal,kellabarugi,ket,sum(rsaldopotensi1) as rsaldopotensi1, statuse');
      $this->db->from('test.ra_dashdb_'.$nama);
      $this->db->where('kellabarugi', $kellabarugi);
      // $this->db->where('tanggal>=', $tglawal);
      $this->db->where('tanggal<=', $tglakhir);
